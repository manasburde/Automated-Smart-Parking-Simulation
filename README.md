...
2. **Compile the code:** (Note: Compiling `Main.java` automatically compiles the files inside the `parking` folder)
...
1. **Park a Vehicle:** Select Option 1 (Car) or Option 2 (Bike). Enter a unique ID (e.g., 101) and the duration in hours.

2. **System Feedback:** The system will confirm the "Slot Number" where the vehicle is parked.

3. **Full Capacity:** If you try to park a 6th vehicle, the system will catch the error and display "Error: Parking Full".

4. **Checkout:** Select Option 3 and enter the Vehicle ID. The system will calculate the total fee and clear the slot for new customers.
